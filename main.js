...
class App {
  ...
  createSim() {
    let fragmentShader = document.getElementById(
      "simulationFragmentShader"
    ).textContent;
    let vertexShader = document.getElementById(
  ...
  onMouseMove(event) {
    let x = (event.clientX / this.winWidth) * 2 - 1;
    let y = -((event.clientY / this.winHeight) * 2 - 1);
  ...
  onTouchMove(event) {
    if (event.touches.length == 1) {
      ...
      let x = (event.touches[0].pageX / this.winWidth) * 2 - 1;
      let y = -((event.touches[0].pageY / this.winHeight) * 2 - 1);
      this.updateMouse(x, y);
    }
  }
  ...
  modifyFloorShader() {
    ...
    let textureMatrix = this.floor.material.uniforms.textureMatrix;
